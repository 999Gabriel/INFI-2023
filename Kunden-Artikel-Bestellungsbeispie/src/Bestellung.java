...
public class Bestellung
{
    ...
    public void createBestellungTable()
    {
        try (PreparedStatement statement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS " +
                "bestellung (kundenID INT, artikelID INT, " +
                "anzahl INT, FOREIGN KEY (kundenID) REFERENCES kunden(id), FOREIGN KEY (artikelID) " +
    ...
    {
        try (PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO bestellung " +
                "(kundenID, artikelID, anzahl) VALUES (?, ?, ?)"))
        {
    ...
    {
        try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM bestellung " +
                "WHERE kundenID = ?"))
        {
            preparedStatement.setInt(1, kundenID);
            ...
            {
                String artikelName = resultSet.getString("artikelID");
                int anzahl = resultSet.getInt("anzahl");
                System.out.println("Kunde " + kundenID + " hat " + anzahl + " Stück(e) von Artikel " +
                        artikelName + " bestellt.");
            }
        } catch (SQLException e)
        {
            e.printStackTrace();
    ...
    public void displayBestellungenWithDate(int idOfOrder) throws SQLException
    {
        ...
        try
        {
            while (resultSet.next())
            {
                System.out.println("KundenID: " + resultSet.getInt("kundenID") + ", ArtikelID: "
                        + resultSet.getInt("artikelID") + ", Anzahl: " +
    ...
    public void getBestellungenInZeitraum(LocalDateTime startDatum, LocalDateTime endDatum)
    {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query))
        {
            ...
            while (resultSet.next())
            {
                ...
                System.out.println("Bestellung ID: " + resultSet.getInt("id") + ", Kunden ID: "
                        + resultSet.getInt("kundenID") +
                        ", Artikel ID: " + resultSet.getInt("artikelID")
    ...
    public void displayBestellungenVonKundeInZeitraum(int kundenID, LocalDate startDatum, LocalDate endDatum)
    {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query))
        {
            ...
            while (resultSet.next())
            {
                ...
                System.out.println("Bestellung ID: " + resultSet.getInt("id") + ", Kunden ID: "
                        + resultSet.getInt("kundenID") +
                        ", Artikel ID: " + resultSet.getInt("artikelID") + ", Anzahl: "
                        + resultSet.getInt("anzahl") +
                        ", Bestellt am: " + resultSet.getTimestamp("bestellt_am"));
            }
        } catch (SQLException e)
