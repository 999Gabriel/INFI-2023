...
public class Artikel {
    ...
    public void createArtikelTable() {
        try (Statement statement = connection.createStatement()) {
            statement.executeUpdate("CREATE TABLE IF NOT EXISTS artikel (id INT AUTO_INCREMENT PRIMARY KEY, bezeichnung VARCHAR(255), preis DECIMAL(10, 2))");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void insertArtikel(String bezeichnung, double preis) {
        try (PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO artikel (bezeichnung, preis, erstellt_am) VALUES (?, ?, NOW())"))
        {
    ...
    public void updateLagerbestand(int artikelID, int anzahl) {
        try (PreparedStatement preparedStatement = connection.prepareStatement("UPDATE artikel SET lagerbestand = lagerbestand - ? WHERE id = ?")) {
    ...
    public void displayArtikelWithDate(int idOfProduct) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                System.out.println("ID: " + resultSet.getInt("id") + ", Bezeichnung: " + resultSet.getString("bezeichnung") + ", Preis: " + resultSet.getDouble("preis") + ", Erstellt am: " + resultSet.getTimestamp("erstellt_am"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void getArtikelNachDatum(LocalDate datum) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                ...
                System.out.println("ID: " + id + ", Bezeichnung: " + bezeichnung + ", Preis: " + preis + ", Erstellt am: " + erstelltAm);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // Weitere Methoden können für das Abrufen von Artikeln implementiert werden
}
